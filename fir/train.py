...
os.environ['CUDA LAUNCH_BLOCKING'] = '1'
import sys
import datetime
import random
import numpy as np
...
def train(Dataset, Network):
    ...
    ## dataset
    cfg = Dataset.Config(datapath='../data/train/HR_DUT/', savepath='./ckpt_fir', mode='train', batch=48, lr=0.05, momen=0.9, decay=5e-4, epoch=80)
    ...
    for epoch in range(cfg.epoch):
        ...
        for step, (image, mask, edge) in enumerate(loader):
            ...
            loss_revo0 = bce_loss(revo0, torch.clamp(up2(gt5) - gt4, 0, 1))
            loss_revo1 = bce_loss(revo1, torch.clamp(up2(gt4) - gt3, 0, 1))
            loss_revo2 = bce_loss(revo2, torch.clamp(up2(gt3) - gt2, 0, 1))
            loss_revo3 = bce_loss(revo3, torch.clamp(gt2 - up0_25(gt), 0, 1))
        ...
        if epoch > cfg.epoch-5 or epoch%5==0:
            torch.save(net.state_dict(), cfg.savepath + '/ESNet_fir_' + str(epoch + 1) + '.pth')
