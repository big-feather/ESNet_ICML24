...
class sec_evo(nn.Module):
    ...
    def forward(self,x,path12,path1_2,l11,l22,mask,mask1):
        ...
        # mask1_ = F.interpolate(mask1, size=l2_a.shape[-2:], mode='bilinear', align_corners=False)
        mask[mask > 0.5] = 1
        mask[mask <= 0.5] = 1e-8
        mask1[mask1 > 0.5] = 1
        ...
        # f_en= self.sfem(f_en,mask,mask1)
...
if __name__ == "__main__":
    ...
    # p_model=trans_decoder().cuda()
    # par = [{'params': t_model.parameters()}, {'params': p_model.parameters()}]
    sec_model=sec_evo().cuda()
    optimizer = torch.optim.Adam(sec_model.parameters(), 0.001)
    CE = torch.nn.BCEWithLogitsLoss()
    # flag=[True]*25+[False]*40
    # b = lr_model.bkbone
